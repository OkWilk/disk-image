...
class RunCommandTest(unittest.TestCase):
    ...
    def test_create_execute_object(self):
        execute = Execute(['ls'])
        self.assertEqual(execute.command, ['ls'])
        self.assertTrue(execute.output_parser)
        self.assertFalse(execute.shell)
        self.assertFalse(execute.use_pty)
        execute = Execute('ls', shell=True)
        self.assertEqual(execute.command, 'ls')
        self.assertTrue(execute.shell)
        execute = Execute(['ls'], use_pty=True)
    ...
    def test_execute_with_pty(self):
        execute = Execute('python3 ./helpers/listprint.py 5 test test2', shell=True, use_pty=True)
